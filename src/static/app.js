  ...
  async function fetchActivities() {
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        // build participants list (with remove icon)
        const participantsHtml = details.participants.length
          ? `<p><strong>Participants:</strong></p><ul class="participants-list">${details.participants
              .map(
                (email) =>
                  `<li>${email} <span class="remove-participant" data-email="${email}" data-activity="${name}">&times;</span></li>`
              )
              .join("")}</ul>`
        ...
        // attach click handlers for any delete icons in this card
  ...
  signupForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        messageDiv.className = "success";
        signupForm.reset();
        // refresh the list so the new participant shows up
        fetchActivities();
      } else {
